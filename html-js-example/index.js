  ...
  if (dom.eventsLogContainer) {
    dom.eventsLogContainer.innerHTML = `
      <div class="empty-state">
        ...
        <div class="empty-state-subtext">Interact with the AR experience to see events appear here</div>
      </div>
    `;
  }
...
function toggleSidebar() {
  ...
  storage.set(CONFIG.STORAGE_KEY, state.isSidebarCollapsed);
}
...
// Note: setVariantsStatus supports custom values for variants:
...
//
// Integration: To sync stock status with your system:
// 1. Use onReady payload to get available products/variants from PulpoAR
// 2. Query your own database/API with the slugs to check stock status
// 3. Call setVariantsStatus with out-of-stock variants
// Example:
//   const pulpoarVariants = onReadyData.products.flatMap(p => p.variants);
//   const outOfStockSlugs = await yourAPI.getOutOfStockProducts(pulpoarVariants.map(v => v.slug));
//   pulpoar.setVariantsStatus({ variants: outOfStockSlugs.map(slug => ({ slug })), isDisabled: true });
function updateStock(event) {
  ...
  try {
    ...
    // Optional: Automatically select the variants in catalog to showcase the marked product
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  const savedState = storage.get(CONFIG.STORAGE_KEY);
  if (savedState === 'true') {
...
window.filterCatalogVariants = () => filterDropdown('catalog-variant-search', 'catalog-variant-select', true);
